...
# Prompt to write ISO to a USB device
echo "==> Optional: Write ISO to a USB drive"
echo "Available removable USB devices (HOTPLUG=1, TRAN=usb):"
lsblk -dpno NAME,SIZE,MODEL,TRAN,HOTPLUG | awk '$5==1 && $4 ~ /usb/ {printf "  %s  %s  %s\n", $1, $2, $3}' || true

read -rp "Enter USB device path (e.g., /dev/sdb) or leave blank to skip: " USB_DEV
if [[ -n "${USB_DEV}" ]]; then
  if [[ ! -b "${USB_DEV}" ]]; then
