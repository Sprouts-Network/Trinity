...
public:
    ...
    // Accessor for internal BIGNUM pointer (const)
    const BIGNUM* get_bn() const { return bn; }
    
    // Accessor for internal BIGNUM pointer (non-const)
    BIGNUM* get_bn() { return bn; }
    ...
    unsigned int GetCompact() const
    {
        ...
        {
            CBigNum bn_temp;
            BN_rshift(bn_temp.bn, bn, 8*(nSize-3));
            nCompact = BN_get_word(bn_temp.bn);
        }
